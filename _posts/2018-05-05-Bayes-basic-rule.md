...
$$
  ...
  我们接下来为了分析的简单一些，都是用公式1作为基础分析展开。我们看公式1$$P(A\vert B) = \frac{P(B\vertA) \cdot P(A)}{P(B)}$$，我习惯这样来看贝叶斯公式，
$$P(A\vert B) = P(A) \frac{P(B\vert A)}{P(B)}$$，这时候可以将贝叶斯公式看为两个部分，第一部分是先验概率P(A)，后一部分是“调整因子”。
...
P(A|B) = P(A)\frac{P(B|A)}{P(B|A)\cdot P(A) + P(B|\bar A)\cdot P(\bar A)} \\
         = 0.001 \cdot \frac{0.99}{0.99 \times 0.001 + 0.01 \times 0.999} \\
     = 0.09016
$$
...
$$ P(A\vert B \bigcap C) = P(A)\frac{P(B \bigcap C|A)}{P(B \bigcap C|A)\cdot P(A) + P(B \bigcap C|\bar A)\cdot P(\bar A)} $$
...
$$P(B \bigcap C\vert A)=0.99 \times 0.99=0.9801$$，已知患病情况下连续两次检测结果为阳性的概率

$$P(B \bigcap C\vert \bar A)=0.01 \times 0.01=0.0001$$，已知未患病情况下连续两次检测结果为阳性的概率

代入后可得：

$$ P(A\vert B \bigcap C) \approx 0.9$$
